...
function crashMrs(xgr, spx) {
    ...
    if (collided) {
        ...
        exp1 = mrSX - train.width;
        exp2 = monsta.y - train.height;

        if (!extralife) {
            fadeOutMusic();
            gamePaused = true;
            reUpdate(180, 2, exp1, exp2, CH / 2);
            const msg = cpop[flor(2)];
            setTimeout(() => showPopup(msg), 1200);
        } else {
            stuck = true;
...
function update(timestamp) {
    ...
    if (stuck) doExp(exp1, exp2, hopo/2, 1);
