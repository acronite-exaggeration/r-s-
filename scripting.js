...
const flor = no => (Math.random() * no) | 0;
...
const offImgs = [imgload("others/cross.png"),imgload("others/burst.webp"),imgload("others/xgr.png")];
const backx = [imgload("bgis/backg000.webp"),imgload("bgis/backg111.webp"),imgload("bgis/backg222.webp"),imgload("bgis/backg333.webp"),imgload("bgis/backg444.webp")];
const trainx = [imgload("prototypes/loco000.webp"),imgload("prototypes/loco111.webp"),imgload("prototypes/loco222.webp"),imgload("prototypes/loco333.webp"),imgload("prototypes/loco444.webp"),imgload("prototypes/loco555.webp")];
const monsterx = [imgload("prototypes/proto000.webp"),imgload("prototypes/proto111.webp"),imgload("prototypes/proto222.webp"),imgload("prototypes/proto333.webp"),imgload("prototypes/proto444.webp"),imgload("prototypes/proto555.webp")];
const obi = [imgload("others/barrel.webp"),imgload("others/barrelx.webp"),imgload("others/crate.jpg"),imgload("others/rock.webp"),imgload("others/scrap.webp"),imgload("others/scrapx.webp")];
const instructions = [
...
let page = 0, what = true, opend = false;
...
let trainMax, acc, fri, mup, monsterBase, monsterMax, difi, tops;
...
let diesel = false, change = false, bgImg, trainImg, monsterImg, trainscale = 3;
...
let colint, textint, interl, currentColor = 1, line = 0, stord = false;
...
let resizeTimer = null, gameRunning = false, gamePaused = false, CW, CH, k, l, editx;
function resiz() {
setViewportHeight();
...
function saveCp() {
const cpData = {rrr: run + Math.floor((reach - CW*9) / (50 * editx)),xp: exp + epo,org: requirement,gt: gpGot,pt: stat.plot + 1};
...
let q;
const query = ["🔥 Why these obstacles...Hmm...","🔥 Im gonna burst all these obstacles...","🔥 Why only me - ***********","🔥 You will pay for this... Monster","🔥 Damn... This monster","🔥 What the Hell - ************"];
function gOver(rg) {
off('gameCanvasX');
off('envy');
setLife(false);
...
let showAdvance = false, advanceAF;
function doAdv() {
if (advanceAF) return;
let a1x = true, a2x = true, a3x = true, a4x = true;
function ina() {
...
let bursting = false, scaleE = 0, lastE = 0, opacityE = 1.5;
...
let flashing = false, opacityF = 0.75, lastF = 0;
function flash(f = 1) {
const var1 = CW;
const var2 = CH;
...
let run = 0, exp = 0, epo = 0, gpgp = true, trackk = 0;
const runup = ele('runnerScore');
function progression(xgr, abc, t) {
if (!challenge) return;
if (gpgp) {
...
crystals.push({x: rand(ending + CW*2),y: 0.36 + rand(0.32),radius: 1,color: cryCol[flor(cryCol.length)]});
}
...
obstacles.push({x: i,y: var4,height: var1,width: scaleww,img: robs,t: topp});
}
for (let i = var2; i < l2; i += (flor(CW/4) + ups)/tops ) {
gpBlocks.push({x: i,y: 0.42 + flor(0.18),width: var3,height: var3,got: false,animating: false,alpha: 1,dy: 0});
}
}
...
let swich = false, startY = 0, targetY = 0, trainBob = 0, crash = false, hopx = 0, bobx = 0;
...
let righto = false, lefto = false, pressT = false, cameraX = 0, speedX = 0, remind = 0, reach = 0, braver = true, braverr = true, lasting = null, stuck = false;
...
let border = 0, challenge = false, onpro = false, offpro = false;
function distanceHandler(kkk, lll) {
const d1 = lll*5.7;
const d2 = kkk - lll*4.7;
...
let lasttime = 0, timer = 400, kb = 0, lastly = 0, traintime = 0, spawner = 20;
...
let uploop, doloop, relasting = 0, allowed = true, reas;
...
progression(reach - topo * 4.5, eepo - topo * 9, timestamp);
...
function startGame() {
const help = ["😤 You can always set Train Speed from Settings","😤 Boost your Reflexes by increasing difficulty & speed","😤 You can always set Monster Difficulty from Settings","🔥 Track & Enable game FPS via settings","😤 You can reset your progress or default the settings","🔥 Always learn from your tiny mistakes","😎 Use Toggle-Music and Pause-Resume feature effectively","😎 Collect Graphene or ExtraLife using 💰 button","😎 Increase your XP + Slow down Monster -- by practicing ADVANCE"];
on('dark');
